-- 1023 — a morada da entrega, e o "sem jalapeño".
--
-- Duas lacunas que o balcão sentiu no primeiro dia de testes.
--
-- 1. MORADA. A entrega vendida ao balcão guardava nome, telefone e zona — e
--    mais nada. O entregador saía com "Zona Maputo" e um telefone, o que não é
--    uma morada. A coluna orders.address já existia desde o motor e a RPC
--    nunca a preenchia: o POS não tinha onde a escrever e o servidor não a
--    lia. Passa a ler.
--
-- 2. NOTA POR ARTIGO. "Sem jalapeño" é meio balcão. O caminho já existia todo
--    — order_items.notes, o payload de impressão e o talão do bridge já a
--    imprimem — só o POS é que nunca a mandava. Não é preciso schema novo:
--    a RPC já lia `notes` de cada item desde a 1012. Fica aqui registado que
--    é deliberado, para ninguém voltar a criar coluna a dobrar.
--
-- 3. O TALÃO DA ENTREGA. Uma entrega vendida ao balcão saía na comanda com o
--    carimbo "BALCÃO", porque o payload mandava orders.channel ('counter') e
--    o que interessa à cozinha e ao entregador é o fulfillment_type. Passam a
--    ir os dois, mais o telefone, a zona e a morada — é o que o entregador
--    precisa de levar na mão.
...
begin
  ...
  if p_kind = 'order' then
    select jsonb_build_object(
      ...
      'channel', o.channel,
      -- O que a cozinha e o entregador leem e o fulfillment: uma entrega
      -- vendida ao balcao tem channel='counter' e saia carimbada BALCAO.
