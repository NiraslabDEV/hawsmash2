-- ============================================================================
-- DELIVERY OS — F2.2: Complete print_job payload for delivery receipts
--
-- Fixes:
-- 1. confirm_payment: creates print_job with full delivery payload
-- 2. advance_order APPROVE: creates print_job with full delivery payload
-- 3. Print_job payload includes all info needed for delivery receipt
--
-- Ref: CLAUDE.md secção 9 (Impressora térmica)
-- ============================================================================

-- ============================================================================
-- ATUALIZA confirm_payment: cria print_job com payload completo de delivery
-- ============================================================================
...
begin
  ...
  -- 2. Lock + leitura do pedido com todos os dados necessários para o cupom
  select 
    o.id, o.status, o.total_cents, o.order_number, o.flow,
    o.fulfillment_type, o.delivery_zone_id, o.address,
    o.customer_name, o.customer_phone, o.scheduled_for,
  ...
  -- 7. Transicionar pedido para 'paid'
  update orders
  set    status = 'paid', updated_at = now()
  where  id = p_order_id;

  -- 8. Buscar nome da zona de entrega (se aplicável)
  if v_order.fulfillment_type = 'delivery' and v_order.delivery_zone_id is not null then
    select name into v_zone_name
    from delivery_zones
    where id = v_order.delivery_zone_id;
  end if;

  -- 9. Construir payload dos itens para o cupom
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
  ...
  -- 10. Criar print_job com payload completo de delivery — CLAUDE.md secção 9
...
-- ============================================================================
-- ATUALIZA advance_order: cria print_job com payload completo no APPROVE
-- ============================================================================
...
begin
  select 
    o.id, o.status, o.flow, o.payment_proof_path, 
    o.customer_name, o.customer_phone, o.fulfillment_type,
    o.delivery_zone_id, o.address, o.scheduled_for,
    o.payment_method, o.notes, o.total_cents, o.created_at
  into v_order
  from orders o
  where o.id = p_order_id;
  ...
  -- CRIAR PRINT_JOB no APPROVE — CLAUDE.md secção 9
  if p_event = 'APPROVE' then
    -- Buscar nome da zona de entrega (se aplicável)
    if v_order.fulfillment_type = 'delivery' and v_order.delivery_zone_id is not null then
      select name into v_zone_name
      from delivery_zones
      where id = v_order.delivery_zone_id;
    ...
    -- Construir payload dos itens para o cupom
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'name', oi.name_snapshot,
    ...
    -- Criar print_job com payload completo de delivery
    insert into print_jobs (order_id, station, payload)
    values (
      p_order_id, 'kitchen',
      jsonb_build_object(
    ...
    -- Log de criação de print_job
    insert into event_log (order_id, type, payload)
    values (
      p_order_id, 'print_job.created',
      jsonb_build_object(
        'trigger', 'advance_order.APPROVE',
        'station', 'kitchen'
      )
    );
  end if;
