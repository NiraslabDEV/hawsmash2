-- ============================================================================
-- Casa do Bom Pasteleiro — Pedido POR PESSOA na mesa (QR).
--
-- No cardápio da mesa, várias pessoas sentadas à mesma mesa pedem no mesmo
-- telemóvel ("A pedir para: Ana / Rui / + Pessoa"). Cada linha do pedido passa
-- a saber de QUEM é, para a comanda da cozinha sair agrupada por pessoa e a
-- equipa servir o prato certo a quem o pediu.
--
-- Decisões fechadas:
--  · `order_items.person_label` é TEXTO LIVRE, opcional (null = pedido da mesa
--    sem distinção). Não há tabela de pessoas: são efémeras, vivem só naquele
--    pedido. Criar uma entidade `diners` seria over-engineering — ninguém
--    consulta "histórico da Ana da mesa 7".
--  · O nome da pessoa NÃO é PII tratada: é uma alcunha que o próprio grupo
--    escreve para se organizar ("Ana", "Rui", "Mano"). Não se liga a
--    `customers` nem a telefone.
--  · `dine_in` deixa de exigir `customerName`: o design da mesa não tem campo
--    de nome (o QR já identifica a mesa). Quando não vier, o pedido fica como
--    'Mesa N' — o painel/comanda continuam a ter um nome legível.
-- ============================================================================

alter table public.order_items
  add column if not exists person_label text;

comment on column public.order_items.person_label is
  'Quem pediu esta linha (só dine_in, texto livre efémero). Null = mesa sem distinção.';

-- ── create_order v8 — person_label por linha + nome opcional no dine_in ─────
...
begin
  ...
  -- Nome: obrigatório em pickup/delivery (é quem levanta/recebe). No dine_in a
  -- mesa já identifica o pedido — o nome é preenchido a seguir com 'Mesa N'.
  if v_fulfillment_type <> 'dine_in'
     and (v_customer_name is null or length(v_customer_name) = 0) then
  ...
  if v_flow = 'dine_in' then
    ...
    v_customer_name := coalesce(v_customer_name, 'Mesa ' || v_table.number);
  else
  ...
  loop
    ...
    -- Quem pediu (só faz sentido no dine_in; nos outros canais é sempre null).
    if v_flow = 'dine_in' then
      v_person := nullif(trim(v_item_el ->> 'person'), '');
    else
      v_person := null;
    end if;
    ...
    insert into order_items (
      ...
      variant_name_snapshot, addons, modifiers, person_label
    ) values (
      v_order_id,
      v_menu_item.id,
      v_menu_item.name,
  ...
  -- pedido (não existe APPROVE/confirm_payment neste fluxo). A comanda sai
  -- ORDENADA POR PESSOA para a cozinha montar prato a prato.
  if v_flow = 'dine_in' then
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
