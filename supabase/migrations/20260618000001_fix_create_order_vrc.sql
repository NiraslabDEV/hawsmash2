-- ============================================================================
-- Fix: "record v_rc is not assigned yet" em create_order sem cupom.
--
-- Causa: PostgreSQL não garante short-circuit em expressões SQL dentro de IF.
-- `if v_referral_code is not null and v_rc.reward_type != 'free_item'` avaliava
-- v_rc.reward_type mesmo quando v_referral_code era null (v_rc não inicializado).
-- Correcção: nested IF garante que v_rc só é acedido após select * into v_rc.
-- ============================================================================
