-- 1021 — a entrega vendida ao balcão passa a ser uma entrega, e a taxa é cobrada.
--
-- Medido em staging: TODAS as vendas de balcão ficavam com
-- fulfillment_type=pickup, delivery_zone_id a null e delivery_fee_cents=0 —
-- incluindo aquelas em que o operador escolheu Entrega e preencheu nome,
-- telefone e zona. O insert escrevia pickup à força e deitava fora o
-- fulfillmentType e o deliveryZoneId que o POS já mandava.
--
-- O que isso custava, por ordem de gravidade:
--   1. A TAXA NUNCA ERA COBRADA. Uma entrega de balcão em Maputo devia somar
--      150 MT. O cliente pagava o subtotal, o sistema registava o subtotal, e
--      a loja perdia a taxa em cada entrega — sem sair no talão, portanto sem
--      ninguém dar por ela.
--   2. O pedido não aparecia nas entregas, porque não era uma entrega.
--   3. Uma zona da Matola era aceite num pedido de Maputo sem se queixar.
--
-- Não é regressão da 1018: o insert vinha assim desde que o POS ganhou o
-- selector de tipo de pedido. O comentário no pos-shell que dizia que a RPC
-- sempre aceitou estes campos era uma suposição que ninguém foi verificar.
--
-- O canal continua counter (é uma venda de balcão). O que muda é o
-- fulfillment_type, que é o que separa o que sai pela porta do que sai ao
-- balcão — e é por ele que a lista de entregas filtra.
