...
begin
  ...
  insert into public.menu_item_variants(id,menu_item_id,name,price_cents,is_default)
    values(variant,item,'Sem açúcar',100000,true);
  ...
  select (v->>'price_cents')::integer into price from jsonb_array_elements(menu->'categories') c,
    jsonb_array_elements(c->'items') i, jsonb_array_elements(i->'variants') v
    where i->>'id'=item::text and v->>'id'=variant::text;
  if price is distinct from 97750 then raise exception 'variante de preço igual divergiu: %',price; end if;
  ...
  v_order_id:=public.create_order('campaign-test',jsonb_build_object(
    'customerName','Cliente teste','customerPhone','841111112','fulfillmentType','pickup','paymentMethod','cash',
    'items',jsonb_build_array(jsonb_build_object('menuItemId',item,'variantId',variant,'qty',1))));
  select total_cents into price from public.orders where id=v_order_id and store_id=s;
  if price is distinct from 97750 then raise exception 'RPC pública divergiu: %',price; end if;
  ...
  select (v->>'price_cents')::integer into price from jsonb_array_elements(menu->'categories') c,
    jsonb_array_elements(c->'items') i, jsonb_array_elements(i->'variants') v
    where i->>'id'=item::text and v->>'id'=variant::text;
  if price is distinct from 115000 then raise exception 'variante após campanha: %',price; end if;
